...
with tab_client:
    if st.session_state.step == "upload":
        ...
        st.markdown(f'<div style="background:rgba(0,0,0,0.4); padding:20px; text-align:center; font-size:24px; border-radius:15px; color: white;">📄 {tot} Page(s) ({st.session_state.nb_g} N/B, {st.session_state.nb_c} Couleur)</div>', unsafe_allow_html=True)
        ...
        if st.button("LANCER L'IMPRESSION", key="btn_print_final"):
            # Ici on valide la commande dans le log
            ...
            st.success("✅ Commande envoyée à l'imprimante !")
            time.sleep(2)
